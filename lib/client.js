...
<body onload="javascript:document.forms[0].submit()">
   ...
   * @api public
   */
  callbackParams(input, idpLoginDebugging) { // eslint-disable-line
    this.debugLogger = idpLoginDebugging ? idpLoginDebugging.logger : null;
    this.debugCategory = idpLoginDebugging ? idpLoginDebugging.category : null;
    this.isIDPLoginDebuggingEnabled = idpLoginDebugging ? idpLoginDebugging.isIDPLoginDebuggingEnabled : false;

    if (this.isIDPLoginDebuggingEnabled) {
      this.debugLogger.info('[IDP Debugging][OIDC] Method client.callbackParams', {
        input: input,
        category: idpLoginDebugging.category
      }); // additional logging
    }
    ...
    if (this.isIDPLoginDebuggingEnabled) {
      this.debugLogger.info('[IDP Debugging][OIDC] Method client.callbackParams, uri in callbackParams', {
        uri: uri,
        category: this.debugCategory
      }); // additional logging
    }
  ...
  authorizationCallback(redirectUri, parameters, checks) {
    if (this.isIDPLoginDebuggingEnabled) { // additional logging
      this.debugLogger.info('[IDP Debugging][OIDC] Method client.authorizationCallback, parameters', {
    ...
    if (toCheck.state !== parameters.state) {
      if (this.isIDPLoginDebuggingEnabled) { // additional logging
        this.debugLogger.info('[IDP Debugging][OIDC] Method client.authorizationCallback, Return promise reject state mismatch', {
          category: this.debugCategory
        });
      }
    ...
    if (params.error) {
      if (this.isIDPLoginDebuggingEnabled) { // additional logging
        this.debugLogger.info('[IDP Debugging][OIDC] Method client.authorizationCallback, return promise reject error in params', {
          category: this.debugCategory
        });
      }
    ...
    if (params.id_token) {
      if (this.isIDPLoginDebuggingEnabled) { // additional logging
        this.debugLogger.info('[IDP Debugging][OIDC] Method client.authorizationCallback, params.id_token', {
          'params.id_token': params.id_token,
          category: this.debugCategory
        });
      }
      ...
      if (this.isIDPLoginDebuggingEnabled) { // additional logging
        this.debugLogger.info('[IDP Debugging][OIDC] Method client.authorizationCallback, params.id_token missing', {
          category: this.debugCategory
        });
      }
    }
    ...
    if (params.code) {
      if (this.isIDPLoginDebuggingEnabled) { // additional logging
        this.debugLogger.info('[IDP Debugging][OIDC] Method client.authorizationCallback, grantCall, params.code present', {
          category: this.debugCategory
        });
      }
      ...
      if (promise) {
        ...
        if (this.isIDPLoginDebuggingEnabled) { // additional logging
          this.debugLogger.info('[IDP Debugging][OIDC] Method client.authorizationCallback, returning grantCall(), promise not present', {
            category: this.debugCategory
          });
        }
    ...
    if (this.isIDPLoginDebuggingEnabled) { // additional logging
      this.debugLogger.info('[IDP Debugging][OIDC] Method client.authorizationCallback, returning promise and grant call in regular return', {
  ...
  authenticatedPost(endpoint, httpOptions) {
    if (this.isIDPLoginDebuggingEnabled) { // additional logging
      this.debugLogger.info('[IDP Debugging][OIDC] Method client.authenticatedPost, before the token endpoint', {
        category: this.debugCategory
      });
    }
    ...
    return Promise.resolve(this.authFor(endpoint))
      .then((auth) => {
        ...
        if (this.isIDPLoginDebuggingEnabled) { // additional logging
          this.debugLogger.info('[IDP Debugging][OIDC] Method client.authenticatedPost', {
            endpoint1: this.issuer[`${endpoint}_endpoint`],
            endpoint2: `${endpoint}_endpoint`,
            opts: opts,
            category: this.debugCategory
          });
        }
  ...
  authFor(endpoint) {
    if (this.isIDPLoginDebuggingEnabled) { // additional logging
      this.debugLogger.info('[IDP Debugging][OIDC] Method client.authFor, getting to the promise before the token endpoint', {
        category: this.debugCategory
      });
    }
  ...
  static register(properties, opts) {
    ...
    if (this.isIDPLoginDebuggingEnabled) { // additional logging
      this.debugLogger.info('[IDP Debugging][OIDC] Method client.register', {
        jwksProperties: properties.jwks,
        keystore: keystore,
        category: this.debugCategory
      });
    }
