...
OpenIDConnectStrategy.prototype.authenticate = function authenticate(req, options) {
  ...
  try {
    ...
    if (options.idpLoginDebugging && options.idpLoginDebugging.isIDPLoginDebuggingEnabled) {
      options.idpLoginDebugging.logger.info('[IDP Debugging][OIDC] isIDPLoginDebuggingEnabled toggle is set to true, debug logging enabled, to disable these logs set isIDPLoginDebuggingEnabled in secrets/providers/bank*/prodivers.json to false.', {
        category: options.idpLoginDebugging.category
      }); // additional logging
    }

    const reqParams = client.callbackParams(req, options.idpLoginDebugging);
