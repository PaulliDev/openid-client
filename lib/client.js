...
class Client {
  ...
  constructor(metadata, keystore) {
    console.log('CLIENT.JS Constructor called'); // additional logging
...
<body onload="javascript:document.forms[0].submit()">
  ...
  callbackParams(input) { // eslint-disable-line
    console.log('1 - entering callbackParams'); // additional logging
    ...
    console.log('1 - uri in callbackParams', uri); // additional logging
  ...
  authorizationCallback(redirectUri, parameters, checks) {
    console.log('3 - authorizationCallback- checks', JSON.stringify(checks)); // additional logging
    console.log('3 - authorizationCallback- parameters', JSON.stringify(parameters)); // additional logging
    console.log('3 - authorizationCallback- redirectUri', redirectUri); // additional logging
    ...
    console.log('3 - authorizationCallback- params', JSON.stringify(params)); // additional logging
    console.log('3 - authorizationCallback- toCheck', JSON.stringify(toCheck)); // additional logging

    if (toCheck.state !== parameters.state) {
      console.log('3 - authorizationCallback- return promise reject state mismatch'); // additional logging

      return Promise.reject(new Error('state mismatch'));
    }

    if (params.error) {
      console.log('3 - authorizationCallback- return promise reject error in params'); // additional logging
    ...
    if (params.id_token) {
      console.log('3 - authorizationCallback- params.id_token', params.id_token); // additional logging
      ...
      console.log('3 - authorizationCallback- params.id_token not there'); // additional logging
    }

    if (params.code) {
      console.log('3 - authorizationCallback- entering grant call bc params.code was there'); // additional logging
      ...
      if (promise) {
        ...
        console.log('3 - authorizationCallback- returning grant call bc there wasnt a promise'); // additional logging

        return grantCall();
      }
    }

    console.log('3 - authorizationCallback- returning promise and grant call in regular return'); // additional logging
  ...
  authenticatedPost(endpoint, httpOptions) {
    console.log('5 - authenticatedPost - entering authenticatedPost before the token endpoint'); // additional logging

    return Promise.resolve(this.authFor(endpoint))
      .then((auth) => {
        const opts = this.issuer.httpOptions(_.merge(httpOptions, auth, { form: true }));

        console.log('5 - authenticatedPost - ${endpoint}_endpoint', this.issuer[`${endpoint}_endpoint`]); // additional logging
        console.log('5 - authenticatedPost - ${endpoint}_endpoint', `${endpoint}_endpoint`); // additional logging
        console.log('5 - authenticatedPost - opts', JSON.stringify(opts)); // additional logging
   ...
   */
  authFor(endpoint) {
    console.log('2 - authFor - getting to the promise before the token endpoint'); // additional logging
  ...
  static register(properties, opts) {
    ...
    console.log(`4 - register - jwks propoerties: ${properties.jwks}`); // additional logging
    console.log(`4 - register - options keystore: ${keystore}`); // additional logging
